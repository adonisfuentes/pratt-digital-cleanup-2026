# `pratt-digital-cleanup-2026`

# Pratt iSchool Digital Cleanup 2026

**Brought to you by PALA, SAA, PEMP, and Patient & Pixels**

**April 10, 2026 | 1:00 PM – 4:00 PM**

**Workshop led by Adonis Fuentes**

---

A hands-on workshop series designed to help students take control of their digital workspace. Each workshop walks through a practical task — from backing up your academic work to organizing your local files — with step-by-step instructions and all the tools you need.

## Workshops

| # | Workshop | Description |
|---|----------|-------------|
| 01 | [Canvas Backup](./Workshop%2001%20(Canvas%20Backup)) | Download and preserve all of your Canvas assignments, materials, and course files. |
| 02 | [Folder Cleanup](./Workshop%2002%20(Folder%20Cleanup)) | Organize your folder structure using the Johnny Decimal system with automation scripts. |

## Getting Started

Each workshop folder contains its own `README.md` with a full step-by-step walkthrough. Start with **Workshop 01** and work through them in order.

### Prerequisites

- A laptop (Mac or Windows)
- A web browser
- Terminal / Command Prompt access

Additional tools may be required per workshop — check each workshop's README for specifics.

## Workshop 01 — Canvas Backup

Back up everything from your Canvas account: assignments, course materials, grades, and more. This workshop uses the [Canvas Student Data Export](https://github.com/davekats/canvas-student-data-export) tool along with Python and NPM to automate the entire process.

**Tools needed:** Terminal, Python 3.8+, Node.js 16+, Browser

➡️ [Go to Workshop 01](./Workshop%2001%20(Canvas%20Backup))

## Workshop 02 — Folder Cleanup & Organization

Learn best practices for organizing your folder structure using the **Johnny Decimal system**. Build a clean, numbered hierarchy for all your files, automate folder creation with scripts, batch-rename existing files to fit your system, and set up tools to maintain it going forward.

**Tools needed:** Terminal, Finder, Homebrew, NPM

➡️ [Go to Workshop 02](./Workshop%2002%20(Folder%20Cleanup))

## License

This project is for educational purposes as part of a live workshop hosted by the Pratt Institute School of Information.
